...
// -------
const lookBack = 3;
const totalDataLength = 750;

// ****
const segmentLength = 25;
const segNumFromEnd = 2;
// -- //
const trainStart = 0;
const trainEnd = totalDataLength - (segNumFromEnd * segmentLength);
const testStart = totalDataLength - (segNumFromEnd * segmentLength);
const testEnd = totalDataLength - ((segNumFromEnd - 1) * segmentLength);
// ****
...
syms.forEach((sym, i) => {
    ...
    dataToUse[sym] = rawData.slice(rawData.length - totalDataLength, rawData.length);
});
...
syms.forEach((sym) => {
    ...
    dataToUse[sym].forEach((snapshot) => {
        ...
        dataLists[sym].push({
            date: snapshot.date,
            close: Number.parseFloat(snapshot.close),
    ...
    dataLists[sym].forEach((snapshot) => {
        let close = Number.parseFloat(snapshot.close);
...
function createAnswerList(dataList, lookBack) {
    ...
    for (let i = lookBack; i < dataList.length - 1; i++) {
        if (dataList[i + 1].close - dataList[i].close < min) {
            min = dataList[i + 1].close - dataList[i].close;
        }
        if (dataList[i + 1].close - dataList[i].close > max) {
            max = dataList[i + 1].close - dataList[i].close;
        }
    ...
    for (let i = lookBack; i < dataList.length - 1; i++) {
        ...
        for (let j = i - lookBack; j < i; j++) {
            if (usePriceDir) {
                if (dataList[j + 1].close > dataList[j].close) {
                    arr.push(1);
                } else {
                    arr.push(0);
                }
